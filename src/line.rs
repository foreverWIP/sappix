        ...
        fn plot_line_common<const HIGH: bool>(
            ...
            if y1 == y2 {
                match color {
                    ...
                    ColorMode::PerPoint([color_a, color_b]) => {
                        for x in x1..x2 {
                            let by = ((((x as u16) << 8) / (((x2 - x1) as u16) << 8)) >> 8) as u8;
            ...
            if x1 == x2 {
                match color {
                    ...
                    ColorMode::PerPoint([color_a, color_b]) => {
                        for y in y1..y2 {
                            let by = ((((y as u16) << 8) / (((y2 - y1) as u16) << 8)) >> 8) as u8;
                            renderer.set(x1, y, color_a.lerp(color_b, by as u8), blend_mode);
                        }
                    }
                }
            }
            ...
            if HIGH {
                match color {
                    ...
                    ColorMode::PerPoint([color_a, color_b]) => {
                        for y in y1..y2 {
                            let by = ((((y as u16) << 8) / (((y2 - y1) as u16) << 8)) >> 8) as u8;
                            renderer.set(x_or_y, y, color_a.lerp(color_b, by as u8), blend_mode);
                            if d > 0 {
                                x_or_y += i;
                                d += (2 * (d_x - d_y)) as i32;
                            } else {
